          ...
          struct fib_stats* stats)
{
    ...
    {
        result = fib_memo (table, n - 1, stats)
               + fib_memo (table, n - 2, stats);
    }
      ...
      char** argv)
{
    ...
    if (n >= 2)
    {
        ...
        printf ("\nfib(%d) looked up again: %llu"
                " (%zu hit%s, %zu miss%s)\n",
                k, (unsigned long long int)cached_value,
                stats.hits, (stats.hits == 1) ? "" : "s",
                stats.misses, (stats.misses == 1) ? "" : "es");
        ...
        printf ("fib(%d) recomputed: %llu (%zu hit%s, %zu miss%s)\n",
                k, (unsigned long long int)recomputed,
                stats.hits, (stats.hits == 1) ? "" : "s",
                stats.misses, (stats.misses == 1) ? "" : "es");
    ...
    printf ("\nfinal cache: %zu entries, capacity %zu\n",
            rh_count (table), rh_capacity (table));
